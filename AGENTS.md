...
- **An IMAP email whose `Date` header cannot be read is treated as undated**: imapflow hands back `envelope.date` as a `Date` when it could parse the header and as the raw header string when it could not, so the desktop IMAP provider parses whatever it gets and falls back to an empty `createdAt` — the same value a message with no envelope date already produced. The alternative, passing the raw header through, is worse than it looks: `createdAt` is compared lexicographically against the last-processed bookmark (see the next bullet), so a non-ISO value would sort against every real timestamp arbitrarily. The consequence is that with "Mark emails as seen" off, such a message is never newer than the bookmark and so is not imported; with it on, the message imports normally and only its `{{date}}` is empty. Real mail from a real server parses — this is the malformed-header path, not the common one.
